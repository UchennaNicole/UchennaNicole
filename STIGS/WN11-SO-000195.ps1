...
try {
    ...
    if ($null -eq $before) {
        ...
        Write-Host "  [FINDING] $RegName = $before — non-compliant value detected." -ForegroundColor Red
    } else {
        Write-Host "  [PASS] $RegName = $before — already compliant." -ForegroundColor Green
    }
}
catch {
...
try {
    ...
    if ($after -eq $RegValue) {
        Write-Host "  [PASS] $RegName = $after — value confirmed compliant." -ForegroundColor Green
    } else {
...
Write-Host ""
Write-Host "  STIG WN11-SO-000195: COMPLIANT" -ForegroundColor Green
Write-Host ""
Write-Host "  NOTE: This setting takes full effect on the next password change." -ForegroundColor Yellow
Write-Host "  Existing LM hashes in the SAM database are not removed until" -ForegroundColor Yellow
Write-Host "  each user account password is reset." -ForegroundColor Yellow
